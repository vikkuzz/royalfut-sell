...
import { CoinRangeCoefficients, PricePolicy } from "@royalfut/collections";
import {
...
export function calculatCoinRange(
    min: number = TransferValidationCredentials.MIN_UT_COINS,
    max: number = TransferValidationCredentials.MAX_UT_COINS
) {
    const result = [];
