...
const Marker: FC<{ label: string }> = ({ label }) => {
    return (
        <span className="inline-block text-center align-middle cursor-pointer text-white-55 text-[0.625rem] whitespace-nowrap font-medium uppercase transition-colors duration-300">
            {label}
        </span>
    );
};
...
const Slider: FC<ISliderProps> = ({
    ...
    return (
        <RCSlider
            ...
            marks={markers}
            step={1}
            ariaLabelForHandle={["Move slider handle"]}
            ...
            classNames={{
                track: "absolute h-[calc(100%_-_0.4375rem)] rounded-[7px] animate-backgroundPan bg-linear-primary-pan background-size-200",
